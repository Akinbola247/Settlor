...
import { createContractExecutionChallenge } from "@/lib/circle-challenges";
import {
...
export async function POST(request: Request) {
  try {
    ...
    const result = await createContractExecutionChallenge(fresh.userToken, {
      walletId,
      contractAddress,
      callData,
      ...(value && value !== "0x0" ? { amount: value } : {}),
    });
    ...
    console.error("[/api/payments/w3s-send-tx]", err);
    const message = err instanceof Error ? err.message : "Could not start transaction";
