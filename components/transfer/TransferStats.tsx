  ...
  return (
    <div className="grid gap-4 sm:grid-cols-3">
      <div className="card relative overflow-hidden p-5">
        <div className="pointer-events-none absolute -right-6 -top-6 h-24 w-24 rounded-full bg-orange-100/80" />
        <p className="text-xs font-bold uppercase tracking-widest text-[var(--color-muted)]">
          Available
        </p>
        <div className="mt-2 flex items-center gap-2">
          <p className="font-serif text-3xl font-bold">${formatUSDC(balance)}</p>
          {syncing && (
            <span className="h-3 w-3 animate-spin rounded-full border-2 border-orange-200 border-t-orange-600" />
          )}
        </div>
      ...
      <div className="card flex flex-col justify-between p-5">
        ...
        <Link href="/dashboard/deposit" className="mt-3 text-xs font-bold text-orange-600 hover:underline">
          Add funds →
        </Link>
      </div>
    </div>
